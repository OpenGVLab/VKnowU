...
class Qwen2VLGRPOVLLMTrainerCaption(Trainer):
    def __init__(
        self,
        ...
        if isinstance(model, str):
            ...
            elif "Qwen2.5-VL" or "VideoRFT" in model_id:
        ...
        ft_llm = getattr(self.script_args, "ft_llm", "")
        if ft_llm != "":
            if ft_llm == "top3":
                layers_to_train = ["model.layers.27.", "model.layers.26.", "model.layers.25.", "merger"]
                ...
                layers_to_train = ["model.layers.0.", "model.layers.1.", "model.layers.2.", "merger"]
                for name, param in model.named_parameters():
                    if any(layer in name for layer in layers_to_train):
                        param.requires_grad = True
                    else:
        ...
        self.beta = args.beta
        # Prefer script_args.beta_strategy if provided via TrlParser(GRPOScriptArguments,...)
        self.beta_strategy = (
            getattr(script_args, "beta_strategy", None)
            or getattr(args, "beta_strategy", None)
            or "default"
        )
        self.visual_knowledge_ratio = (
            getattr(script_args, "visual_knowledge_ratio", None)
            or getattr(args, "visual_knowledge_ratio", None)
            or 1.0
        )
        print("--------------------------------")
        print("beta_strategy: ", self.beta_strategy)
        print(f"visual_knowledge_ratio: {self.visual_knowledge_ratio}")
        print("--------------------------------")
    ...
    def compute_loss(
        ...
        data_type = inputs[-1]['data_type']
        
        if data_type == 'image':
            input_copy[-1]['content'][0]['image'] = inputs[0]['path']
        elif data_type == 'video':
            input_copy[-1]['content'][0]['video'] = inputs[0]['path']
        ...
        if self.args.use_vllm:
            ...
            # !bug https://github.com/tulerfeng/Video-R1/issues/23#issuecomment-2938272232
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            reward_kwargs['visual_knowledge_ratio'] = self.visual_knowledge_ratio
        ...
        elif self.beta_strategy == 'default':
            beta = self.beta
        ...
        # Make metrics computation robust to edge cases (e.g., last microbatch, uneven sizes)
        if gathered_rewards.dim() == 0:
            gathered_rewards = gathered_rewards.view(1)
        total_elements = gathered_rewards.numel()
        if total_elements >= self.num_generations and total_elements % self.num_generations == 0:
            num_devices = total_elements // self.num_generations
            # Safe reshape: will not error due to divisibility check above
            rewards_per_device = gathered_rewards.view(num_devices, self.num_generations)
            wrong_devices = (rewards_per_device <= 1).all(dim=1)
            wrong_ratio = wrong_devices.sum().item() / max(1, num_devices)

            correct_devices = (rewards_per_device >= 2).all(dim=1)
            correct_ratio = correct_devices.sum().item() / max(1, num_devices)

            self._metrics["all_wrong"].append(wrong_ratio)
            self._metrics["all_correct"].append(correct_ratio)
        else:
            # When grouping is not possible, record neutral values to avoid crashing the training loop
            self._metrics["all_wrong"].append(0.0)
            self._metrics["all_correct"].append(0.0)
