...
class Qwen2VLGRPOLoraTrainer(Trainer):
    """
    ...
    def __init__(
        ...
        vision_lora = bool(getattr(self.script_args, "vision_lora", False))
        llm_lora = bool(getattr(self.script_args, "llm_lora", False))
        ...
        if vision_lora or llm_lora:
            lora_namespan_exclude = ['lm_head', 'embed_tokens']
            if vision_lora and llm_lora:
                pass
            elif vision_lora:
                lora_namespan_exclude.append('model.layers')
            elif llm_lora:
                lora_namespan_exclude.append('visual.')
            ...
            for name, param in model.named_parameters():
                if vision_lora and llm_lora:
                    pass
                elif vision_lora and "model.layers" in name:
                    param.requires_grad = False
                elif llm_lora and "visual.blocks" in name: # 只训练LLM lora
                    param.requires_grad = False
                if "merger" in name:
            ...
            print(f"[LoRA] 可训练参数: {trainable_params} / 总参数: {total_params} | 可训练比例: {(trainable_params / total_params*100):.2f}%")


        #self.ref_model = None
        # Reference model
        if peft_config is None and not vision_lora and not llm_lora:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Clone the inference tensor to make it a normal tensor for backward pass
        ref_per_token_logps = ref_per_token_logps.clone()
