...
parser.add_argument('--mode', type=str, required=True, help="Mode: VAN or COT")
parser.add_argument('--frames', type=str, default="None", help="Frames")
parser.add_argument('--pixels', type=str, default="None", help="Pixels")
...
for dataset_name in ['vknowu']:
    if args.frames != "None":
        OUTPUT_PATH = f"./eval_results/eval_{dataset_name}/{args.mode}_{MODEL_PATH.replace('/', '_')}_{args.frames}_{args.pixels}.json"
    else:
        OUTPUT_PATH = f"./eval_results/eval_{dataset_name}/{args.mode}_{MODEL_PATH.replace('/', '_')}.json"

    output_dir = os.path.dirname(OUTPUT_PATH)
    if output_dir:
        os.makedirs(output_dir, exist_ok=True)

    PROMPT_PATH = f"./eval_data/eval_{dataset_name}.json"
    ...
    for x in data:
        ...
        if args.mode == "COT":
            custom_prompt = QUESTION_TEMPLATE.format(Question=question) + TYPE_TEMPLATE["multiple choice"]
        elif args.mode == "SEE_THINK":
            custom_prompt = SEE_THINK_TEMPLATE.format(Question=question)
        elif args.mode == "VAN":
            custom_prompt = f"{question}\n" + TYPE_TEMPLATE["multiple choice"]
        # print(custom_prompt)
        elif args.mode == "MIMO":
            custom_prompt = f"{question}\n" + (
                "You are provided a multiple-choice question and a video. Determine the answer to the question based on the video.\n"
                "First provide an internal step-by-step reasoning between <think> </think> tags. \n"
                "Then provide the single option letter (e.g., A, B, C, D, E, etc.) between the <answer> </answer> tags. \n"
                "The output format should be: <think> reasoning process here </think><answer> answer here </answer>."
            )
        else:
            raise ValueError(f"Invalid mode: {args.mode}")

        video_path = "./VKnowU/videos/" + x["qid"] + ".mp4"
    ...
    for i in tqdm(range(start_idx, len(messages), BSZ), desc="Processing batches"):
        ...
        try:
            ...
            print("video_inputs[0].shape:", video_inputs[0].shape)
